...
test ('Data driven test using excel file', async ({page}) => {
    ...
    for (let i = 2; i <= worksheet.rowCount; i++){
        ...
        const actualText = await page.locator("div.alert-message.warning").innerText();
        //expect (actualText).toContain('has been created');
        const expectedText = "has been created"
        if (actualText.includes(expectedText)) {
            console.log(" ***** The computer is added successfully")
            page.screenshot("AddedSuccessfully.png");
        } else {
            console.log(" ***** The computer is NOT added ")
            page.screenshot("NotAddedSuccessfully.png");
        }
